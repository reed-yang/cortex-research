...
// cortex-local edit: this file diverges from the shadcn/assistant-ui registry
// item. The Delete action was replaced by Archive (Cortex archives threads and
// never deletes them), and the touch targets below `lg` were raised to the
// 44 CSS-pixel minimum the phone surface requires -- `New Thread`, each thread
// row and the row's More-options button carry `min-h-11`/`min-w-11` with an
// `lg:` reset back to the registry's compact sizes, and the row's reserved
// end padding follows the wider button. Below `lg` the More-options button is
// also always visible and the row always reserves its space: the registry
// reveals it on hover, which a phone has no way to do, so a 44-pixel invisible
// target would otherwise swallow the end of every row's tap. The desktop hover
// reveal is unchanged, as are the glyphs. Re-apply all of it after any registry
// re-pull.
  ...
  return (
    <ThreadListPrimitive.New asChild>
      <Button
        ...
        className={cn(
          "hover:bg-muted data-active:bg-muted h-8 min-h-11 justify-start gap-2 rounded-md px-2.5 text-sm font-normal lg:min-h-8",
          className,
        )}
        {...props}
      >
...
export const ThreadListItem: FC = () => {
  ...
  return (
    <ThreadListItemPrimitive.Root
      ...
      className="group hover:bg-muted focus-visible:bg-muted data-active:bg-muted has-focus-visible:bg-muted has-data-[state=open]:bg-muted relative flex h-8 min-h-11 items-center rounded-md transition-colors focus-visible:outline-none lg:min-h-8"
    >
      {isRenaming ? (
        ...
        <ThreadListItemPrimitive.Trigger
          ...
          className="focus-visible:ring-ring/50 flex h-full min-w-0 flex-1 items-center rounded-md px-2.5 text-start text-sm outline-none max-lg:pe-12 group-hover:pe-12 group-has-focus-visible:pe-12 group-has-data-[state=open]:pe-12 group-data-active:pe-12 focus-visible:ring-1 lg:group-hover:pe-9 lg:group-has-focus-visible:pe-9 lg:group-has-data-[state=open]:pe-9 lg:group-data-active:pe-9"
        >
          {isRunning && (
            <Loader2Icon
              aria-hidden
    ...
    <ThreadListItemMorePrimitive.Root sharedFocusGroup>
      <ThreadListItemMorePrimitive.Trigger asChild>
        <Button
          ...
          className="data-[state=open]:bg-accent absolute end-1.5 top-1/2 size-6 min-h-11 min-w-11 -translate-y-1/2 p-0 opacity-0 max-lg:opacity-100 group-hover:opacity-100 group-has-focus-visible:opacity-100 group-data-active:opacity-100 data-[state=open]:opacity-100 lg:min-h-6 lg:min-w-6"
        >
      ...
      >
        ...
        {/* Cortex archives threads and never deletes them, so the registry's
            Delete action is left out of this menu. */}
