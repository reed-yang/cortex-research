...
/* eslint-disable react-hooks/refs -- upstream registry helper: a render-time ref
   cache that keeps the markdown component map referentially stable. */
...
// Stable plugin identities avoid rebuilding the parser on unrelated renders.
const remarkPlugins = [remarkGfm, remarkMath];
const rehypePlugins: NonNullable<React.ComponentProps<typeof MarkdownTextPrimitive>["rehypePlugins"]> = [
  [rehypeKatex, { trust: false, maxExpand: 1000, maxSize: 20, output: "htmlAndMathml" }],
];

const MarkdownTextImpl: FC<MarkdownTextProps> = ({ components, smooth = true }) => {
