...
from review import ReviewError, parse_findings
...
def sync(source, repo):
    ...
    if not policy.get("custom_branch_policies") or len(policies) != 1 or (
        policies[0].get("name") != "main" or policies[0].get("type") != "branch"
    ):
        raise AgyError("protected_environment_must_allow_only_main_branch")
    ...
    print("Updated AGY_OAUTH_JSON in the main-only pr-review environment.")
